    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        for(int i = 0; i < tasks; i++) {
            ...
            threads[i] = new Thread(() -> {
                ...
                for(int tempI = 0; tempI < iterations; tempI++) {
                    int left = counterI * (n / tasks) + 1;
                    threadPrivateMyNew[left] = (threadPrivateMyVal[left - 1] + threadPrivateMyVal[left + 1]) / 2.0;

                    int right = (counterI + 1) * (n / tasks);
                    ...
                    double[] tempThreadArray = threadPrivateMyNew;
                    threadPrivateMyNew = threadPrivateMyVal;
                    threadPrivateMyVal = tempThreadArray;
                }
            });
            threads[i].start();
        }
